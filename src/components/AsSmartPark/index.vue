...
onMounted(async () => {
  ...
  /* Object */
  gltfLoader.load("static/models/smartPark/building_8.glb", (gltf) => {
    const model = gltf.scene;
    scene.add(model);
  });

  gltfLoader.load("static/models/smartPark/metro.glb", (gltf) => {
    const model = gltf.scene;
    scene.add(model);
  });
  gltfLoader.load("static/models/smartPark/other_building.glb", (gltf) => {
    const model = gltf.scene;
    scene.add(model);
  });
  /* Lights */
  ...
  createGsapAnimation(camera.position, new THREE.Vector3(60, 100, 60));
